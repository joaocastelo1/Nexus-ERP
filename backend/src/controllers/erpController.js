...
exports.getProducts = async (req, res) => {
  try { res.json(await dataStore.getProducts()); }
...
exports.createProduct = async (req, res) => {
  try { res.status(201).json(await dataStore.createProduct(req.body)); }
  catch (error) { res.status(400).json({ error: error.message }); }
};

exports.updateProduct = async (req, res) => {
  try {
    const product = await dataStore.updateProduct(req.params.id, req.body);
    if (!product) return res.status(404).json({ message: 'Produto não encontrado' });
    res.json(product);
  } catch (error) { res.status(400).json({ error: error.message }); }
};
...
exports.deleteProduct = async (req, res) => {
  try {
    await dataStore.deleteProduct(req.params.id);
...
exports.createSale = async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (product.quantity < item.quantity) return res.status(400).json({ message: `Estoque insuficiente para ${product.name}` });
    }
    ...
    const sale = await dataStore.createSale({ clientId, clientName: client.name, items: saleItems, subtotal, tax, total, paymentMethod, notes });
...
exports.updateSaleStatus = async (req, res) => {
  try {
    const updated = await dataStore.updateSaleStatus(req.params.id, req.body.status);
...
exports.getTransactions = async (req, res) => {
  try { res.json(await dataStore.getTransactions()); }
  catch (error) { res.status(500).json({ error: error.message }); }
};

exports.createTransaction = async (req, res) => {
  try { res.status(201).json(await dataStore.createTransaction(req.body)); }
  catch (error) { res.status(400).json({ error: error.message }); }
};
...
exports.getFinanceSummary = async (req, res) => {
  try {
    ...
    res.json(await dataStore.getFinanceSummary(period));
  } catch (error) { res.status(500).json({ error: error.message }); }
};
